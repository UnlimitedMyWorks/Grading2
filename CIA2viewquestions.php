<?php include('header.php');include("config.php");?>
      ...
      <div id="page-wrapper">
         <div class="main-page">
            <h2 class="title1">CIA 2 Questions</h2>
            <form method = "post">
                ...
                <tbody>
                    ...
                    <?php
                        if(isset($_POST['coursecode']))
                        {
                           ...
                           {
                              $group=$_POST['group'];
                              $sql="SELECT * FROM `cia2questions` where coursecode='".$var."' and groupforfirstyear='$group' ORDER BY CAST(qno AS UNSIGNED)";
                           }
                           else
                           {
                              $sql="SELECT * FROM `cia2questions` where coursecode='".$var."' ORDER BY CAST(qno AS UNSIGNED)";
                           }
                           $result= $mysqli->query($sql);
                           $counter=mysqli_num_rows($result);
                           if($counter > 0)
      ...
      <?php
         if(isset($_POST['createtable']))
         {
               ...
               $query = "SELECT DISTINCT(qno) from cia2questions ORDER BY CAST(qno AS UNSIGNED)";
               ...
               $result=$mysqli->query("SELECT 1 FROM cia2 LIMIT 1");
               if($result)
               {
                  $mysqli->query($sqlq);
                  ...
                  $col=substr($col,0,-1);
                  $sql="INSERT INTO cia2temp (".$col.") SELECT ".$col." FROM cia2";
                  $res=$mysqli->query($sql);
                  if($res)
                  {
                        $mysqli->query("DROP TABLE IF EXISTS cia2");
                        $mysqli->query("RENAME TABLE cia2temp to cia2");
                        ...
                        $csv_handler = fopen ('files\cia2markentry.csv','w');
         ...
         if(isset($_POST['delqno']))
         {
            ...
            $qno=$_POST['delqno'];
            $mysqli->query("DELETE FROM cia2questions where coursecode='$coursecode' AND qno='$qno'");
         ...
         {
            $coursecode=$_POST['coursecode'];
            $mysqli->query("DELETE FROM cia2questions where coursecode='$coursecode'");
            echo "<script > window.location('cia2viewquestions.php');</script>";
         }
