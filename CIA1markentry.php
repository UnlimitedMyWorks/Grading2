...
if(isset($_POST['submit']))
{
  ...
  if (mysqli_num_rows($result) == 0) {
      ...
      echo "<script>alert('Select Proper Class!!');window.location.href='CIA1selectsubject.php';  </script>";
  }
}
      ...
      <div class="panel-body widget-shadow">
        <div class="row">
          <form class="form-horizontal"  method="post" name="upload_excel" enctype="multipart/form-data">
            <fieldset>
              <!-- Form Name -->
              <legend>Upload CIA 1 Marks </legend>
              <!-- File Button -->
...
if (isset($_POST["Import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
        $count  = 0;
        $sql    = "SELECT * from cia1 where sem='$sem' AND section='$sec' and dept='$dept' and coursecode='$coursecode'  AND staffid='$staffid'";
        $result = $mysqli->query($sql);
        if ($result)
            $mysqli->query("DELETE from cia1 where sem='$sem' AND section='$sec' and dept='$dept' and coursecode='$coursecode' AND staffid='$staffid'");
        $file = fopen($filename, "r");
        if ($coursecode == "CSE201") {
            $sql = "SELECT DISTINCT(qno) from cia1questions WHERE coursecode='$coursecode' and groupforfirstyear='$group' ORDER BY CAST(qno AS UNSIGNED)";
        } else {
            $sql = "SELECT DISTINCT(qno) from cia1questions WHERE coursecode='$coursecode' ORDER BY CAST(qno AS UNSIGNED)";
            
        }
        $totalsql  = '';
        $result    = $mysqli->query($sql);
        $ctr       = mysqli_num_rows($result);
        $insertsql = "INSERT INTO `cia1`(`Reg_no`, `student_name`, `coursecode`, `sem`, `section`, `dept`, `staffid`,";
        for ($i = 0; $i < $ctr; $i++) {
        ...
        while (($getData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($ln == 0) {
                ...
                $sql2 = " ('" . TRIM($getData[0]) . "','" . $getData[1] . "','$coursecode','$sem','$sec','$dept','$staffid',";
        ...
        if (!isset($result)) {
            ...
            if ($result)
                echo "<script>alert('CSV sucessfully Uploaded!!');window.location.href='CIA1selectsubject.php';  </script>";
            else
                echo "<script>alert('CSV not  sucessfully Uploaded!!');window.location.href='CIA1selectsubject.php';  </script>";
        }
