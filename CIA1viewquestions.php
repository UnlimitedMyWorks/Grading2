<?php include('header.php');include("config.php");?>
      ...
      <div id="page-wrapper">
         <div class="main-page">
            <h2 class="title1">CIA 1 Questions</h2>
            <form method = "post">
                ...
                <tbody>
                    ...
                    <?php
                        if(isset($_POST['coursecode']))
                        {
                           ...
                           {
                              $group=$_POST['group'];
                              $sql="SELECT * FROM `cia1questions` where coursecode='".$var."' and groupforfirstyear='$group' ORDER BY CAST(qno AS UNSIGNED)";
                           }
                           else
                           {
                              $sql="SELECT * FROM `cia1questions` where coursecode='".$var."' ORDER BY CAST(qno AS UNSIGNED)";
                           }
                           $result= $mysqli->query($sql);
                           $counter=mysqli_num_rows($result);
                           if($counter > 0)
      ...
      <?php
         if(isset($_POST['createtable']))
         {
               ...
               $query = "SELECT DISTINCT(qno) from cia1questions ORDER BY CAST(qno AS UNSIGNED)";
               ...
               $result=$mysqli->query("SELECT 1 FROM cia1 LIMIT 1");
               if($result)
               {
                  $mysqli->query($sqlq);
                  ...
                  $col=substr($col,0,-1);
                  $sql="INSERT INTO cia1temp (".$col.") SELECT ".$col." FROM cia1";
                  $res=$mysqli->query($sql);
                  if($res)
                  {
                        $mysqli->query("DROP TABLE IF EXISTS cia1");
                        $mysqli->query("RENAME TABLE cia1temp to cia1");
                        ...
                        $csv_handler = fopen ('files\cia1markentry.csv','w');
         ...
         if(isset($_POST['delqno']))
         {
            ...
            $qno=$_POST['delqno'];
            $mysqli->query("DELETE FROM cia1questions where coursecode='$coursecode' AND qno='$qno'");
         ...
         {
            $coursecode=$_POST['coursecode'];
            $mysqli->query("DELETE FROM cia1questions where coursecode='$coursecode'");
            echo "<script > window.location('CIA1viewquestions.php');</script>";
         }
