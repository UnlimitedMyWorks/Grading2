<?php include('header.php');include("config.php");?>
      ...
      <div id="page-wrapper">
         <div class="main-page">
            <h2 class="title1">Assignment 2 Questions</h2>
            <form method = "post">
            <div class="form-group">
									<label for="Coursecode" class="col-sm-2 control-label">Course Name</label>
									<div class="col-sm-8"><select name="coursecode"  class="form-control1" >
                ...
                <tbody>
                    ...
                    <?php
                        if(isset($_POST['coursecode']))
                        {
                           ...
                           $sql="SELECT * FROM `assignment2questions` where coursecode='".$var."' ORDER BY CAST(qno AS UNSIGNED)";
                           $result= $mysqli->query($sql);
                           $counter=mysqli_num_rows($result);
                           if($counter > 0)
                           {
      ...
      <?php
         if(isset($_POST['createtable']))
         {
               ...
               $query = "SELECT DISTINCT(qno) from assignment2questions ORDER BY CAST(qno AS UNSIGNED)";
               ...
               $sqlq="Create table assignment2temp(
                  Reg_no INT(15),
               ...
               $result=$mysqli->query("SELECT 1 FROM assignment2 LIMIT 1");
               if($result)
               {
                  $mysqli->query($sqlq);
                  ...
                  $col=substr($col,0,-1);
                  $sql="INSERT INTO assignment2temp (".$col.") SELECT ".$col." FROM assignment2";
                  echo $sql;
                  $res=$mysqli->query($sql);
                  if($res)
                  {
                        $mysqli->query("DROP TABLE IF EXISTS assignment2");
                        $mysqli->query("RENAME TABLE assignment2temp to assignment2");
                        ...
                        $csv_handler = fopen ('files\assignment2markentry.csv','w');
         ...
         if(isset($_POST['delqno']))
         {
            ...
            $qno=$_POST['delqno'];
            $mysqli->query("DELETE FROM assignment2questions where coursecode='$coursecode' AND qno='$qno'");
         ...
         {
            $coursecode=$_POST['coursecode'];
            $mysqli->query("DELETE FROM assignment2questions where coursecode='$coursecode'");
            echo "<script > window.location('assignment2viewquestions.php');</script>";
         }
